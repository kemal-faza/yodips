...
describe("ScheduleSection", () => {
  it("renders the plain card title without the IRS suffix or caption", () => {
    const w = mount(ScheduleSection, {
      props: { items, semester: "Ganjil 2025/2026", loading: false },
    });
    expect(w.text()).toContain("Jadwal Kuliah");
    // The 'IRS' suffix is gone, and the old caption line is removed.
    expect(w.text()).not.toContain("IR");
    expect(w.text()).not.toContain("SIAP Undip");
  });
  it("shows unparsable rows in the table view", async () => {
    const w = mount(ScheduleSection, {
      props: { items, semester: null, loading: false },
    });
