...
// Explicit token-sync wiring: the auth store installs no import-time
// subscription, so bootstrap starts it once and every rotation lands in
// setToken (the one guarded write path).
useAuthStore(pinia).initTokenSync();
