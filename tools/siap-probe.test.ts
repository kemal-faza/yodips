...
function fakeRes(opts: { status?: number; url?: string; body?: string; contentType?: string }): FakeResponse {
  ...
  return {
    ...
    clone: () => ({ arrayBuffer: async () => enc.buffer.slice(enc.byteOffset, enc.byteOffset + enc.byteLength) }),
  };
}

function withFetch(mock: () => unknown, fn: () => Promise<void>): Promise<void> {
...
describe('siap-probe', () => {
  ...
  it('maps a 200 HTML response to a PreviewResponse with stripped preview', async () => {
    await withFetch(
      () => fakeRes({ status: 200, body: '<html><h1>Hi</h1> Berita jadwal</html>', contentType: 'text/html' }),
      async () => {
        const r = await probe('https://siap.undip.ac.id/jadwal', { method: 'GET' });
        assert.equal(r.status, 200);
