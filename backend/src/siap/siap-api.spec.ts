...
describe('SiapApiUpstream', () => {
  ...
  it.each([
    ...
    await expect(api.fetch(endpoint, 'T', {}, '24060121130000')).resolves.toEqual({ endpoint });
  ...
  it('accepts a successful mint payload on a 3xx response and records one attempt', async () => {
    ...
    await expect(api.mintToken('x@y', '24060121130000')).resolves.toMatchObject({ token: 'T3' });
  ...
  it('maps a generic mint 5xx failure to api-endpoint 502', async () => {
    ...
    const err = await api.mintToken('x@y', '24060121130000').catch((e) => e);
    expect(err).toBeInstanceOf(StaleUpstreamError);
