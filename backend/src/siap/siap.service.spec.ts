...
describe('SiapService', () => {
  ...
  describe('sub-based session resolution (endpoint API)', () => {
      ...
      return new SiapService(
        ...
        undefined,
        {
          get: jest.fn().mockResolvedValue(
            cookie ? { siapCookie: cookie, identity: '24060121130000', emailSso: 'x@students.undip.ac.id' } : null,
          ),
          set: jest.fn(),
        } as any,
        apiMock as any,
    ...
    it('resolves the SIAP identity from SessionStore by sub and drives the API', async () => {
      ...
      apiMock.fetch
        .mockResolvedValueOnce({ nama: 'Budi', nim: '24060121130000', nama_ps: 'TI', namafak: 'FSM', tahun_masuk: '2024', sso_email: 'x@students.undip.ac.id', status_terakhir: 'Aktif' })
      ...
      expect(apiMock.mintToken).toHaveBeenCalledWith('x@students.undip.ac.id', '24060121130000');
    });
  ...
  describe('getNotifications', () => {
    const apiMock = { mintToken: jest.fn(), fetch: jest.fn() };
    const sessionStore = {
      get: async () => ({ siapCookie: 's', identity: '24060121130000', emailSso: 'x@students.undip.ac.id' }),
      set: jest.fn(),
    };
...
describe('API-backed methods', () => {
  ...
  it('fallback-scrapes emailSso from fetchProfile when session lacks it', async () => {
    ...
    const svc = new SiapService(
      ...
      {
        get: async () => ({ siapCookie: 's', identity: '24060121130000' }),
        set: jest.fn(),
      } as any,
      apiMock as SiapApiUpstream,
    );
    apiMock.mintToken.mockResolvedValue({ token: 'T', data: {} });
    apiMock.fetch
      .mockResolvedValueOnce({ nama: 'Budi', nim: '24060121130000', sso_email: 'x@students.undip.ac.id' })
      .mockResolvedValueOnce({ nm_smt: '2026/2027 Ganjil' });
    const p = await svc.getProfile('u1');
    expect(apiMock.mintToken).toHaveBeenCalledWith('x@students.undip.ac.id', '24060121130000');
    expect(p.nama).toBe('Budi');
