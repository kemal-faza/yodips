      ...
      expect(fetchMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          body: expect.stringContaining(
            encodeURIComponent('10622041#24060121130000#460110'),
          ),
        }),
      );
    });
  ...
  describe('getKhs', () => {
    ...
    it('merges the web get_khs detailId (id#nim#kode) onto API rows by kode', async () => {
      ...
      expect(sem2.nilai[0]).toMatchObject({
        id: '10622041',
        kode: 'MIK1624203',
        detailId: '10622041#24060121130000#460110',
      });
      expect(sem2.nilai[1].detailId).toBe('10622042#24060121130000#460149');
...
describe('API-backed methods', () => {
  ...
  it('fallback-scrapes emailSso via the seam when session lacks it', async () => {
    ...
    const storeStub = {
      get: async () => ({ siapCookie: 's', identity: '24060121130000', sessionGeneration: TEST_GEN, capturedAt: Date.now() }),
      getIfGeneration: async (_s: string, g: string) =>
        g === TEST_GEN ? { siapCookie: 's', identity: '24060121130000', sessionGeneration: TEST_GEN, capturedAt: Date.now() } : null,
    } as any;
