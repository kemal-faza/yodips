...
describe('swrWindow', () => {
  it('derives fresh from CachePolicy and stale from defaultStaleTtlMs', () => {
    expect(swrWindow('KULON_COURSES')).toEqual({
      freshTtlMs: 300_000,
      staleTtlMs: 600_000,
    });
    expect(swrWindow('SIAP_KHS')).toEqual({
      freshTtlMs: 1_800_000,
      staleTtlMs: 1_800_000,
    }); // capped 30min
    expect(swrWindow('SIAP_LECTURERS')).toEqual({
...
describe('classifyCacheKey', () => {
  const validCases = [
    ...
    [
      '123:kulon:assignments:all',
      {
        ...
        policyKey: 'KULON_ASSIGNMENTS_ALL',
        swr: true,
      },
    ],
    [
      '123:kulon:assignment-detail:42',
      {
    ...
    [
      '123:kulon:course-content:456',
      {
        label: 'kulon.course_content',
        policyKey: 'KULON_COURSE_CONTENT',
        swr: true,
      },
    ],
    [
      '123:kulon:sesskey:abcdef0123456789', // gitleaks:allow test fixture
      { label: 'kulon.sesskey', policyKey: 'KULON_SESSKEY', swr: false },
    ],
    [
      '123:siap:profile',
      { label: 'siap.profile', policyKey: 'SIAP_PROFILE', swr: true },
    ],
    ['123:siap:irs', { label: 'siap.irs', policyKey: 'SIAP_IRS', swr: true }],
    ['123:siap:khs', { label: 'siap.khs', policyKey: 'SIAP_KHS', swr: true }],
    [
      '123:siap:lecturers',
      { label: 'siap.lecturers', policyKey: 'SIAP_LECTURERS', swr: true },
    ],
    [
      '123:siap:notifications',
      {
        label: 'siap.notifications',
        policyKey: 'SIAP_NOTIFICATIONS',
        swr: true,
      },
    ],
    [
      '123:siap:jadwal',
      { label: 'siap.jadwal', policyKey: 'SIAP_JADWAL', swr: true },
    ],
    [
      '123:siap:absen',
      { label: 'siap.absen', policyKey: 'SIAP_ABSEN', swr: true },
    ],
    [
      '123:siap:identity',
      { label: 'siap.identity', policyKey: 'SIAP_IDENTITY', swr: false },
    ],
    [
      '123:siap:token',
      { label: 'siap.token', policyKey: 'SIAP_TOKEN', swr: false },
    ],
  ] as const;
  ...
  const invalidCases = [
    ...
    [
      'invalid numeric assignment suffix',
      'x:kulon:assignment-detail:not-a-number',
    ],
    ...
    [
      'fingerprint resource suffix',
      'x:kulon:sesskey:abcdef0123456789:resource',
    ],
  ...
  it('supports valid multi-atom subjects without substring matching', () => {
    ...
    expect(
      classifyCacheKey('org~2:tenant-01:kulon:assignment-detail:42'),
    ).toEqual({
      label: 'kulon.assignment_detail',
      policyKey: 'KULON_ASSIGNMENT_DETAIL',
      swr: true,
