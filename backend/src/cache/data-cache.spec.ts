...
async function withInMemory(fn: (c: InMemoryDataCache) => Promise<void>): Promise<void> {
  const c = new InMemoryDataCache(60_000);
  await fn(c);
}
async function withRedis(fn: (c: RedisDataCache) => Promise<void>): Promise<void> {
...
function ageInMemory(c: InMemoryDataCache, key: string, ageMs: number): void {
  const entries = (c as unknown as { entries: Map<string, { fetchedAt: number }> }).entries;
...
describe('getStale (shared behavior)', () => {
  it('miss → sync fetch, stale=false', async () => {
    await withInMemory(async (c) => {
      ...
      const r = await c.getStale('k', fetcher, { freshTtlMs: 10_000, staleTtlMs: 20_000 });
  ...
  it('fresh → cached, no refresh', async () => {
    await withInMemory(async (c) => {
      ...
      const r = await c.getStale('k', fetcher, { freshTtlMs: 60_000, staleTtlMs: 120_000 });
  ...
  it('stale → serves stale immediately + exactly one background refresh', async () => {
    await withInMemory(async (c) => {
      ...
      const r = await c.getStale('k', fetcher, { freshTtlMs: 30_000, staleTtlMs: 120_000 });
  ...
  it('expired → sync fetch (no stale serve)', async () => {
    await withInMemory(async (c) => {
      ...
      await new Promise((r) => setTimeout(r, 5));
      const fetcher = jest.fn().mockResolvedValue('new');
      const r = await c.getStale('k', fetcher, { freshTtlMs: 0, staleTtlMs: 0 });
  ...
  it('background 401 (dead session) → key deleted; next get is miss', async () => {
    await withInMemory(async (c) => {
      ...
      await c.getStale('k', fetcher, { freshTtlMs: 30_000, staleTtlMs: 120_000 });
      await new Promise((r) => setTimeout(r, 5));
...
describe('handleBackgroundError', () => {
  it('dead-session reasons → hard-expire', async () => {
    ...
    for (const reason of ['login-redirect', 'no-cookie', 'api-credential', 'redirect-loop', 'html-content-type', 'malformed-json', 'no-emailSso', 'non-json-process']) {
      const e = new StaleUpstreamError('Siap', reason);
      await handleBackgroundError({ del } as never, 'k', e);
    }
  ...
  it('transient reasons → keep (no del)', async () => {
    ...
    for (const reason of ['fetch-threw', 'api-endpoint']) {
      await handleBackgroundError({ del } as never, 'k', new StaleUpstreamError('Kulon', reason));
    }
    // http-not-ok with upstream 500+ → statusForStaleReason maps to 502 → transient
    await handleBackgroundError({ del } as never, 'k', new StaleUpstreamError('Kulon', 'http-not-ok', undefined, { status: 502 } as Response));
    expect(del).not.toHaveBeenCalled();
  });
  it('http-not-ok with upstream 4xx → statusForStaleReason maps to 401 → dead-session (hard-expire)', async () => {
    const del = jest.fn().mockResolvedValue(undefined);
    await handleBackgroundError({ del } as never, 'k', new StaleUpstreamError('Kulon', 'http-not-ok', undefined, { status: 401 } as Response));
    expect(del).toHaveBeenCalledTimes(1);
  });
  it('no-api-upstream → transient (no del)', async () => {
    const del = jest.fn().mockResolvedValue(undefined);
    await handleBackgroundError({ del } as never, 'k', new StaleUpstreamError('Siap', 'no-api-upstream'));
    expect(del).not.toHaveBeenCalled();
  });
});
