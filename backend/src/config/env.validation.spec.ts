...
describe('validateEnv', () => {
  ...
  it('defaults TRUST_PROXY_HOPS to 0 when absent (fail-safe: no forwarded-header trust)', () => {
    const cfg = validateEnv({
      SSO_BASE_URL: 'https://sso.undip.ac.id',
      JWT_SECRET: TEST_JWT_SECRET,
      MS_TENANT_ID: 'tenant',
  ...
  it('accepts TRUST_PROXY_HOPS=1 (one trusted proxy) and TRUST_PROXY_HOPS=2 (two proxies)', () => {
    ...
    expect(cfg1.TRUST_PROXY_HOPS).toBe(1);
    ...
    expect(cfg2.TRUST_PROXY_HOPS).toBe(2);
  });

  it('rejects TRUST_PROXY_HOPS outside 0..2', () => {
    const base = {
      SSO_BASE_URL: 'https://sso.undip.ac.id',
      JWT_SECRET: TEST_JWT_SECRET,
      MS_TENANT_ID: 'tenant',
